...
新 handler 使用 TC39 class 装饰器注册（从 `@/core/dispatch/decorators/index.js` 导入）：

```typescript
import { type Context } from '@/core/dispatch/context.js'
import { Handler, OnCommand, OnKeyword, Scope, Permission, SettingNode } from '@/core/dispatch/decorators/index.js'
...
- `@Startup` — 标记初始化方法，由 `LifecycleOrchestrator` 按依赖图排序调用

基础设施 key（可通过 `@Inject` 注入）：`db`、`chat_db`、`cache`、`persistent`、`cache_redis`、`persistent_redis`、`bot_client`、`msg_api`、`group_api`、`friend_api`、`file_api`、`system_api`、`extension_api`、`dispatcher`、`queue`

业务服务 key（由 `@Provide` 注册后可通过 `@Inject` 使用）：`oss`、`media_storage`、`task_scheduler`、`chat_service`、`archive_service`、`renderer`、`settings`、`settings_checker`、`personnelService` 等
...
│   ├── lifecycle/   # 生命周期编排（orchestrator、decorators、service-registry）
...
│   ├── bot-client.ts  # NapCat SDK 客户端启动注册（BotClientBootstrap）
...
### NapCat 连接管理 (`src/core/bot-client.ts`)

NapCat 主动反向 WebSocket 连接 Aemeath。`@aemeath-projects/napcat` SDK 的 `ReverseWebSocketTransport` 负责侦听与连接管理，`NapCatClient` 代理 Transport 并提供类型安全事件分发。`BotClientBootstrap` 通过 `@Provide` 将 `NapCatClient` 及 6 个 API 模块（`msg_api`、`group_api`、`friend_api`、`file_api`、`system_api`、`extension_api`）注册到服务注册表。
...
| `NAPCAT_WS_PORT`       | `3001`               | NapCat 反向 WebSocket 侦听端口（与 `PORT` 必须不同） |
...
| `S3_ARCHIVE_BUCKET`    | `aemeath-archive`   | 聊天归档 bucket                        |
